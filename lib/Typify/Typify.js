...
class Typify {
     ...
     * @protected
     *
     * @throws {InvalidArgumentException} If the given type definition is not valid.
    ...
    static _parseType(type){
        ...
        if ( index > 0 ){
            ...
            if ( type.charAt(type.length - 1) !== '>' ){
                throw new InvalidArgumentException('Malformed type definition.', 2);
            }
     ...
     * @param {Object.<string, (string|string[])>} values An object containing the values to convert.
     ...
     * @param {boolean} [skipMissing=false] If set to "true" missing properties will be ignored, otherwise, once a missing property is found, null will be returned as the process result.
     *
     ...
     * @throws {InvalidArgumentException} If one of the given types has not been registered.
     * @throws {InvalidArgumentException} If one of the given type definitions is not valid.
     * @throws {InvalidArgumentException} If one of the given type definitions is malformed or not valid.
     * @throws {InvalidArgumentException} If the a value is not a string nor an array of strings.
     */
    static typify(properties, values, skipOptionals = false, skipMissing = false){
        ...
        for ( let name in properties ){
            if ( properties.hasOwnProperty(name) ){
                ...
                if ( values[name] === '' || values[name] === null || typeof values[name] === 'undefined' ){
                    ...
                    if ( Array.isArray(values[name]) ){
                        results[name] = [];
                        let length = values[name].length, i = 0, current;
                        // Iterate all the elements contained in the array then cast them one by one.
                        while ( i < length && current !== null ){
                            if ( typeof values[name][i] !== 'string' ){
                                throw new InvalidArgumentException('Invalid value: string expected.', 3);
                            }
                            current = Typify._castValue(values[name][i], typeProperties);
                            results[name].push(current);
                            i++;
                        }
                    ...
                    if ( results[name] === null && skipMissing !== true ){
     ...
     * @throws {InvalidArgumentException} If the a value is not a string nor an array of strings.
     */
    static typifyMulti(properties, values, skipOptionals = false, skipMissing = false){
        if ( properties === null || typeof properties !== 'object' ){
            throw new InvalidArgumentException('Invalid object mapping.', 1);
        }
        ...
        for ( let i = 0 ; i < length ; i++ ){
            castedValues.push(Typify.typify(properties, values[i], skipOptionals, skipMissing));
        }
        return castedValues;
    }
