...
const DriverNotConnectedException = require('../../Exceptions/DriverNotConnectedException');
const DuplicateEntryException = require('../../Exceptions/DuplicateEntryException');
...
class MemcachedCacheDriver extends CacheDriver {
    ...
    static async setup(){
        ...
        for (let i = 0 ; i < cache._connections.length ; i++ ){
            ...
            // Ignoring configuration block that are not referred to Memcached driver.
            ...
            try{
                if ( Array.isArray(driver.cluster) && driver.cluster.length > 0 ){
                    ...
                    let cluster = await MemcachedClusteredConnection.createFromConfigBlock(driver.cluster);
                    if ( cluster.hasConnections() ){
                        await cluster.connect();
                        MemcachedCacheDriver.addConnection(driver.name, cluster);
                    }
                    continue;
                }
                // Generating the connection class instance from the configuration block.
                let connection = MemcachedConnection.createFromConfigBlock(driver);
                // Establish the connection.
                await connection.connect();
                // Register the connection globally.
                MemcachedCacheDriver.addConnection(driver.name, connection);
            }catch(ex){}
        }
        return true;
    }
    ...
    static addConnection(name, connection){
        ...
        Cache.addConnection('memcached', name, connection);
    }
    ...
    setConnection(connection){
        if ( connection === null ){
            ...
            connection = Cache.getConnection('memcached', null);
            if ( connection === null ){
        ...
        if ( connection !== '' && typeof connection === 'string' ){
            ...
            connection = Cache.getConnection('memcached', connection);
            if ( connection === null ){
    ...
    async setMulti(items, options){
        ...
        // Ensure the connection to the _storage engine and prepare the keys.
        ...
        if ( options.overwrite ){
            keys.forEach((key, index) => {
                if ( items.hasOwnProperty(key) ) {
                    processes.push(new Promise((resolve, reject) => {
                        connection.set(processedKeys[index], items[key], ttl, (error) => {
                            if ( typeof error !== 'undefined' ){
                                return reject(error);
                            }
                            resolve();
            ...
            keys.forEach((key, index) => {
                if ( items.hasOwnProperty(key) ) {
                    processes.push(new Promise((resolve, reject) => {
                        connection.add(processedKeys[index], items[key], ttl, (error) => {
                            if ( typeof error !== 'undefined' ){
                                return reject(error.notStored === true ? new DuplicateEntryException('Key already existing.', 4) : error);
                            }
                            resolve();
