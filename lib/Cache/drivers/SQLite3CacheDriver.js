...
/**
 * @typedef {CacheSetOptions} CacheSQLite3SetOptions The options accepted by the SQLite3 driver during "set" operations.
 *
...
/**
 * @typedef {CacheSetOptions} CacheSQLite3GetOptions The options accepted by the SQLite3 driver during "set" operations.
 *
...
/**
 * @typedef {CacheExistsOptions} CacheSQLite3ExistsOptions The options accepted by the SQLite3 driver during "exists" operations.
 */

/**
 * @typedef {CacheExpireOptions} CacheSQLite3ExpireOptions The options accepted by the SQLite3 driver during "expire" operations.
 */

/**
 * @typedef {CacheRemoveOptions} CacheSQLite3RemoveOptions The options accepted by the SQLite3 driver during "remove" operations.
 */

/**
 * @typedef {CacheInvalidateOptions} CacheSQLite3InvalidateOptions The options accepted by the SQLite3 driver during "invalidate" operations.
 *
 * @property {boolean} [dropFile=false] If set to "true" the database file will be removed, otherwise all its entries will be removed and the file kept.
 */

/**
 * @typedef {CacheIncrementOptions} CacheSQLite3IncrementOptions The options accepted by the SQLite3 driver during "increment" operations.
 *
...
/**
 * @typedef {CacheDecrementOptions} CacheSQLite3DecrementOptions The options accepted by the SQLite3 driver during "decrement" operations.
 *
...
/**
 * @typedef {CacheSetMultiOptions} CacheSQLite3SetMultiOptions The options accepted by the SQLite3 driver during multiple "set" operations.
 *
...
/**
 * @typedef {CacheOptions} CacheSQLite3GetMultiOptions The options accepted by the SQLite3 driver during multiple "get" operations.
 *
 * @property {boolean} [silent=false] If set to "true" and if the given item doesn't exist an exception will be thrown, otherwise, null will be returned instead.
 */

/**
 * @typedef {CacheExistsMultiOptions} CacheSQLite3ExistsMultiOptions The options accepted by the SQLite3 driver during multiple "exists" operations.
 */

/**
 * @typedef {CacheRemoveMultiOptions} CacheSQLite3RemoveMultiOptions The options accepted by the SQLite3 driver during multiple "remove" operations.
 */

/**
 * @typedef {CacheIncrementMultiOptions} CacheSQLite3IncrementMultiOptions The options accepted by the SQLite3 driver during multiple "increment" operations.
 *
 * @property {boolean} [create=false] If set to "true" and if the element doesn't exist it will be created applying the increment to zero.
 * @property {boolean} [silent=false] If set to "true" and if the element doesn't exist or it isn't a number it will be ignored, otherwise an exception will be thrown.
 */
...
/**
 * @typedef {CacheDecrementMultiOptions} CacheSQLite3DecrementMultiOptions The options accepted by the SQLite3 driver during multiple "decrement" operations.
 *
 * @property {boolean} [create=false] If set to "true" and if the element doesn't exist it will be created applying the decrement to zero.
 * @property {boolean} [silent=false] If set to "true" and if the element doesn't exist or it isn't a number it will be ignored, otherwise an exception will be thrown.
 */
...
/**
 * @typedef {CacheExpireMultiOptions} CacheSQLite3ExpireMultiOptions The options accepted by the SQLite3 driver during multiple "expire" operations.
 */
...
class SQLite3CacheDriver extends CacheDriver {
     ...
     *
     * @returns {Promise<CachingKeyComponents>} An object containing the generated key components such as the key and the namespace hash.
     *
     * @throws {DriverNotDefinedException} If no connection to the cache storage engine has been defined.
     * @throws {DriverNotConnectedException} If the driver that has been defined is not connected to the cache storage engine.
     ...
     * @protected
     */
    async _prepareTransaction(key, options){
        // Checking that a connection has been defined and that is ready to be used.
        await this._ensureConnection();
     ...
     * @param {?CacheOptions} options The same options object that has been passed to the caller method.
     *
     * @returns {Promise<MultiCachingKeyComponents>} An object containing the generated components including the key hashes.
     *
     * @throws {DriverNotDefinedException} If no connection to the cache storage engine has been defined.
     * @throws {DriverNotConnectedException} If the driver that has been defined is not connected to the cache storage engine.
     ...
     * @protected
     */
    async _prepareMultipleTransaction(keys, options){
        // Checking that a connection has been defined and that is ready to be used.
        await this._ensureConnection();
        // Ensure that the database has the required tables.
        await this._ensureTables();
     ...
     * @param {?CacheSQLite3SetOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
    ...
    async set(key, value, options = null){
        const components = await this._prepareTransaction(key, options);
     ...
     * @param {?CacheSQLite3GetOptions} [options] An object containing some additional options to consider.
     *
     ...
     * @param {?CacheSQLite3ExistsOptions} [options] An object containing some additional options to consider.
     *
     ...
     * @param {?CacheSQLite3ExpireOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
     ...
     *
     * @param {string} key A string representing the element's key.
     * @param {?CacheSQLite3RemoveOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
     ...
     *
     * @param {?CacheSQLite3InvalidateOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
    ...
    async invalidate(options = null){
        ...
        await (new Promise((resolve, reject) => {
            ...
            this._connection.getConnection().run(query, [this._getComputedNamespace(options)], (error) => {
                if ( error !== null ){
                    return reject(new RuntimeException('Unable to complete the operation on the database side.', 3, error));
                }
                resolve();
     ...
     * @param {?CacheSQLite3IncrementOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
     ...
     * @param {string} key A string representing the element's key.
     * @param {?number} [value=1] A floating point number representing the increment delta (positive or negative), the default value is 1.
     * @param {?CacheSQLite3DecrementOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
    ...
    async decrement(key, value = 1, options = null){
        await this.increment(key, ( value === null || isNaN(value) ? -1 : -value ), options);
    }
     ...
     * @param {?CacheSQLite3SetMultiOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
    ...
    async setMulti(items, options = null){
        ...
        const components = await this._prepareMultipleTransaction(keys, options);
        ...
        const ttl = this.getComputedTTL(options);
        const expire = SQLite3CacheDriver._computeExpireDate(ttl);
        //OPTIMIZE: Optimize this bulk insertion.
        await (new Promise((resolve, reject) => {
     ...
     * @param {?CacheSQLite3GetMultiOptions} [options] An object containing some additional options to consider.
     *
     ...
     * @param {?CacheSQLite3ExistsMultiOptions} [options] An object containing some additional options to consider.
     *
     ...
     *
     * @param {string[]} keys A sequential array of strings containing the keys.
     * @param {?CacheSQLite3RemoveMultiOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
     ...
     * @param {string[]} keys A sequential array of strings containing the keys.
     * @param {?number} [value=1] A floating point number representing the increment delta (positive or negative), the default value is 1.
     * @param {?CacheSQLite3IncrementMultiOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
     ...
     * @param {string[]} keys A sequential array of strings containing the keys.
     * @param {?number} [value=1] A floating point number representing the increment delta (positive or negative), the default value is 1.
     * @param {?CacheSQLite3DecrementMultiOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
     ...
     * @param {?CacheSQLite3ExpireMultiOptions} [options] An object containing some additional options to consider.
     *
     * @returns {Promise<void>}
     *
     * @throws {DriverNotDefinedException} If no connection to a SQLite3 database has been defined.
