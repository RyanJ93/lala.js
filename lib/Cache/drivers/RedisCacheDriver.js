...
class RedisCacheDriver extends CacheDriver {
     ...
     *
     * @returns {Promise<boolean>} If the required external module has been found will be returned "true", otherwise "false".
     *
     * @async
     */
    static async setup(){
        ...
        if ( cache === null || cache.connections === null || typeof cache.connections !== 'object' ){
            return true;
        }
        const length = cache.connections.length;
        for ( let i = 0 ; i < length ; i++ ){
            const driver = cache.connections[i];
     ...
     *
     * @param {object} block An object that represents the connection to generate.
     *
     * @throws {InvalidArgumentException} If the given object is not a valid configuration block.
     */
    static createConnectionFromConfigBlock(block){
        return RedisConnection.createFromConfigBlock(block);
    }

    /**
     * Generates a clustered connection object from a given configuration object obtained from a configuration file.
     *
     * @param {object[]} block A sequential array that contains the objects that represent the connections to use within the clustered connection.
     *
     * @throws {InvalidArgumentException} If the given object is not a valid configuration block.
     */
    static createClusteredConnectionFromConfigBlock(block){
        return RedisClusteredConnection.createFromConfigBlock(block);
    }
