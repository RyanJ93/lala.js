...
/**
 * The main class for cache management.
 */
class Cache extends EventEmitter {
    ...
    static async initFromConfig(){
            ...
            for ( const name in connections ){
                if ( name !== '' && typeof name === 'string' && connections.hasOwnProperty(name) && CacheDriverRepository.has(name) ){
                    if ( connections[name] !== '' && typeof connections[name] === 'string' ){
                        ...
                        try{
                            // Generate the connection based on the given driver, then set it as default connection for that driver.
                            const connection = ConnectionFactoryHelper.createFromConfigBlock(name, connections[name]);
                            Cache.setDefaultDriverConnection(name, connection);
                        }catch(ex){
                            Logger.log('[Cache]: Configuration found for the default connection for driver "' + name + '" is not valid.');
                        }
                    }
                }
            }
        ...
        const instances = cache.instances === null || typeof cache.instances !== 'object' ? {} : cache.instances;
        // Loop through all the cache instances defined in the configuration block than must be created and registered as globally available providers.
        for ( const name in instances ){
            if ( instances.hasOwnProperty(name) && name !== '' && typeof name === 'string' ){
                if ( typeof instances[name] === 'string' && instances[name] !== '' ){
                    // This instance must inherit from a registered template.
                    if ( CacheTemplateRepository.has(instances[name]) ){
                        // Creating the cache instance based on the given template.
                        const cache = new Cache(instances[name]);
                        // Register it globally.
                        cache.register(name, true);
                    }
                }else if ( instances[name] !== null && typeof instances[name] === 'object' ){
                    // This instance has been declared manually, using a template as a helper to create the cache object to register.
                    const template = await CacheTemplate.createFromConfigBlock(instances[name]);
                    template.buildCacheObject().register(name, true);
                }
            }
        }
    }
     ...
     *
     * @param {(null|string|CacheTemplate)} [template=null] A string representing the name of the template to use to configure this class instance, alternatively, an instance of the class "CacheTemplate" can be directly used, by default the default template is used.
     *
    ...
    constructor(template = null){
        ...
        /**
         * @type {string} _driver A string representing the name of the cache driver to use.
         *
         * @private
         */
        this._driver = '';
        ...
        /**
         * @type {object|null} _driverInstance The instance of the defined cache driver class.
         *
         * @private
         */
        ...
        /**
         * @type {{string: *}|null} _driverOptions An object containing the custom options that will be passed to the driver class instance.
         *
         * @private
         */
        ...
        /**
         * @type {string|null} _namespace A string containing the namespace of the stored items.
         *
         * @private
         */
    ...
    /**
     * Registers this class instance making it available project wide, this method is chainable.
     *
     * @param {string} name A string containing an unique name that this instance will be registered with.
     * @param {boolean} [overwrite=false] If set to "true" and if another instance having that same name has already been registered will be replaced, otherwise an exception will be thrown.
     *
     * @returns {Cache}
     *
    ...
    register(name, overwrite = false){
        CacheRepository.register(name, this, overwrite);
        return this;
    }
     ...
     *
     * @param {{string: *}} options An object containing the custom options that the driver should consider.
     */
     ...
     *
     * @returns {{string: *}} options An object containing the custom options that the driver should consider.
     */
    getDriverOptions(){
     ...
     *
     * @returns {string} A string representing the driver name.
     */
    getDriver(){
        return this._driver;
    }
     ...
     *
     * @returns {Promise<*>} The original value returned by the driver's method.
     *
    ...
    set(key, value, options = null){
        ...
        return this._call('set', key, value, options);
    }
     ...
     */
     get(key, options = null){
        ...
        return this._call('get', key, options);
    }
     ...
     */
     exists(key, options = null){
        ...
        return this._call('exists', key, options);
    }
     ...
     *
     * @param {string} key A string representing the element's key.
     * @param {?CacheExistsOptions} [options] An object representing the additional options to consider.
     *
     * @returns {Promise<boolean>} If the key was found will be returned "true", otherwise "false".
     *
     * @throws {InvalidArgumentException} If the given key is not valid.
     ...
     */
     expire(key, expire, options = null){
        ...
        return this._call('expire', key, expire, options);
    }
     ...
     */
     remove(key, options = null){
        ...
        return this._call('remove', key, options);
    }
     ...
     */
     invalidate(options = null){
        return this._call('invalidate', options);
    }
     ...
     */
     increment(key, value = 1, options = null){
        return this._call('increment', key, value, options);
    }
     ...
     */
     decrement(key, value = 1, options = null){
        return this._call('decrement', key, value, options);
    }
     ...
     */
     setMulti(items, options = null){
        return this._call('setMulti', items, options);
    }
     ...
     */
     getMulti(keys, options = null){
        return this._call('getMulti', keys, options);
    }
     ...
     * @async
     */
     existsMulti(keys, all = false, options){
        return this._call('existsMulti', keys, all, options);
    }
     ...
     * @async
     */
     hasMulti(keys, all = false, options){
        return this._call('existsMulti', keys, all, options);
    }
     ...
     */
     removeMulti(keys, options = null){
        return this._call('removeMulti', keys, options);
    }
     ...
     */
     incrementMulti(keys, value, options = null){
        return this._call('incrementMulti', keys, value, options);
    }
     ...
     */
     decrementMulti(keys, value, options = null){
        return this._call('decrementMulti', keys, value, options);
    }
     ...
     */
     expireMulti(keys, expire, options){
        return this._call('expireMulti', keys, expire, options);
    }
}
