...
const InterceptorRunner = require('./InterceptorRunner');
...
class Server extends EventEmitter {
    ...
    /**
     * Runs all the interceptors defined in this server's interceptor runner for a given request processing phase.
     *
     ...
     *
     * @throws {ForbiddenHTTPException} If current client request should be blocked according to executed interceptors.
     *
     * @async
     * @protected
     */
    async _runInterceptors(breakpoint, request, response){
        const consent = await this._interceptorRunner.run(breakpoint, request, response);
        if ( consent !== true ){
            throw new ForbiddenHTTPException('Request was rejected by interceptors.', 1);
        }
    }
    ...
    async _preprocessRequest(request, response){
        ...
        await this._runInterceptors('request.preprocess', request, response);
    ...
    async _prepareRequest(request, response){
        // Checking if the request can continue according to interceptors.
        await this._runInterceptors('request.prepare', request, response);
        const processor = this._inputProcessorFactory.craft();
        await processor.process(request, response);
    ...
    async _checkAuthorization(request, response){
        // Checking if the request can continue according to interceptors.
        await this._runInterceptors('request.authorization', request, response);
    ...
    async _processOutput(request, response){
        // Checking if the request can continue according to interceptors.
        await this._runInterceptors('request.outputProcess', request, response);
    ...
    _completeRequest(request, response){
        setImmediate(async () => {
            ...
            this.emit('request.cleanupComplete', request, response);
        });
        this.emit('request.cleanup', request, response);
    }
    ...
    constructor(){
        ...
        /**
         * @type {?Object} [_server=null] An object representing the server implementation instantiated from a native or external module.
         *
         * @protected
         */
        this._server = null;
        ...
        /**
         * @type {OutputProcessorFactory} _outputProcessorFactory An instance of the class "OutputProcessorFactory" used to generate instances of the class that will handle client responses.
         *
         * @protected
         */
        ...
        /**
         * @type {ExceptionProcessorFactory} _exceptionProcessorFactory An instance of the class "ExceptionProcessorFactory" used to generate instances of the class that will handle exceptions.
         *
         * @protected
         */
        ...
        /**
         * @type {InterceptorRunner} _interceptorRunner An instance of the "InterceptorRunner" class containing all the interceptors to run for the incoming client requests.
         *
         * @protected
         */
        this._interceptorRunner = new InterceptorRunner();
    }
    ...
    /**
     * Sets the interceptor runner to use during request processing, this method is chainable.
     *
     * @param {InterceptorRunner} interceptorRunner An instance of the class "InterceptorRunner".
     *
     * @returns {Server}
     *
     * @throws {InvalidArgumentException} If an invalid interceptor runner instance is given.
     */
    setInterceptorRunner(interceptorRunner){
        if ( !( interceptorRunner instanceof InterceptorRunner ) ){
    ...
    /**
     * Returns the interceptor runner being used in request processing.
     *
     * @returns {InterceptorRunner} The instance of the class "InterceptorRunner" being used.
     */
    getInterceptorRunner(){
        return this._interceptorRunner;
    }
