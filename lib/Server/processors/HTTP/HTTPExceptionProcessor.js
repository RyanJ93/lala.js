 ...
 * @property {Map<string, BaseViewFactory>} errorPages A map containing all the error pages to show up whenever a given exception occurs, each entry key must be a string containing the exception name and its value must be a "BaseViewFactory" object.
 * @property {Map<number, BaseViewFactory>} errorPagesByCode A map containing all the error pages to show up whenever a HTTP error occurs, each entry key must be a number representing the HTTP status code and its value must be a "BaseViewFactory" object.
 * @property {?BaseViewFactory} defaultErrorPage An instance of the class "BaseViewFactory" representing the page to show when an exception occurs and no custom page has been defined for it nor its corresponding HTTP status code.
 */
...
class HTTPExceptionProcessor extends ExceptionProcessor {
     ...
     *
     * @returns {?BaseViewFactory} An instance of the class "BaseViewFactory" representing the page to show up, if no page has been defined, null will be returned instead.
     *
     * @protected
     */
    ...
    constructor(configuration = null){
        ...
        /**
         * @type {Map<string, BaseViewFactory>} _errorPages A map containing all the pages to show when the exception, which name is used as entry key, occurs.
         *
         * @protected
         */
        ...
        /**
         * @type {Map<number, BaseViewFactory>} _errorPagesByCode A map containing all the pages to show when a particular exception occurs, exception is defined by its corresponding HTTP status code.
         *
         * @protected
         */
        ...
        /**
         * @type {?BaseViewFactory} _defaultErrorPage The view representing the page to show when an exception occurs and no custom page has been defined for it nor its corresponding HTTP status code.
         *
         * @protected
         */
    ...
    async process(exception, request, response){
        ...
        if ( page !== null ){
            if ( page instanceof ParametrizedViewFactory ){
                // Merge error parameters with the ones defined in the given response object.
                const params = typeof response.viewParams === 'object' ? Object.assign(response.viewParams, errorData) : errorData;
                // Inject the status code and message as parameters to the view.
                params.exception = exception;
                params.isHTTPException = exception instanceof HTTPException;
                params.debugEnabled = Logger.getDebug();
                // Generate the view object from the factory defined.
                output = page.craft(params, new Context(request, response));
            }else{
                // Generate the view object from the factory defined.
                output = page.craft();
            }
...
Object.defineProperty(HTTPExceptionProcessor, 'DEFAULT_ERROR_VIEW', {
    value: new ViewFactory(__dirname + '/../../resources/default_error_page.ejs'),
    writable: false,
    configurable: false
});
