...
class OutputProcessor extends Processor {
    ...
    _getSupportedCompressionAlgorithms(){
        ...
        if ( this._compression === true ){
            let contentType = typeof this._headers['Content-Type'] === 'string' ? this._headers['Content-Type'] : this._contentType;
            if ( contentType !== null && contentType !== '' ){
                // Extract the content type (MIME type) only: mime/type; charset="UTF-8".
                const index = contentType.indexOf(';');
                if ( index > 0 ){
                    contentType = contentType.substr(0, index).trim();
                }
            }
            // Check if response is already a compressed entity.
            if ( contentType === null || contentType === '' || OutputProcessor.COMPRESSED_ENTITY_CONTENT_TYPES.indexOf(contentType) === -1 ){
    ...
    /**
     * Generates the content type header according to the content type and charset detected.
     *
     ...
     * @private
     */
    _computeContentTypeHeader(response){
        let contentType = null, charset = null;
        ...
        if ( contentType !== null ){
            ...
            if ( response.charset !== '' && typeof response.charset === 'string' ){
                charset = response.charset;
            }else if ( this._charset !== '' ){
    ...
    _prepareHeaders(response){
        if ( this._stealth !== true ){
            this._headers['X-Powered-By'] = Logger.getDebug() ? ( 'Lala.js/' + VERSION ) : 'Lala.js';
        }
        if ( response.hasHeader('content-type') ){
            this._headers['Content-Type'] = response.getHeader('content-type');
        }else{
            const contentType = this._computeContentTypeHeader(response);
            if ( contentType !== null ){
    ...
    async _serialize(data){
        ...
        this._charset = serializer.getCharset();
