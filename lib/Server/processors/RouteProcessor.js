 ...
 * @property {?CORSOptions} [CORSOptions] An instance of the class "CORSOptions" containing the CORS configuration to use globally.
 */
...
class RouteProcessor extends Processor {
    ...
    static _generateCacheKey(request){
        let key = null, languageSignature = '';
        if ( request.server !== null && typeof request.server === 'object' ){
            ...
            key = request.server.getID() + ':' + request.method + ':' + request.path + languageSignature;
        }
        return key;
    }
    ...
    _generateRequestLanguagesList(request){
        ...
        const languages = [];
        if ( request.declaredLanguage !== '' && typeof request.declaredLanguage === 'string' ){
            // Add the language that has been found using language declaration in request processor.
            languages.push(request.declaredLanguage);
        }
        if ( request.preferredLanguage !== '' && typeof request.preferredLanguage === 'string' ){
            ...
            languages.push(request.preferredLanguage);
        }
        if ( request.languages !== null && typeof request.languages === 'object' ){
            // Add all the other languages specified by the user (HTTP header).
            const genericCodes = [];
            for ( const code in request.languages ){
                // Check if it is a locale code (for instance "en-US"), if it is then extract the language code (for instance "en").
                const index = code.indexOf('-');
                if ( index > 0 ){
                    genericCodes.push(code.substr(0, index));
                }
            ...
            for ( let i = 0 ; i < length ; i++ ){
                languages.push(genericCodes[i]);
            }
        }
        if ( this._defaultLanguage !== null ){
            languages.push(this._defaultLanguage);
        }
        // Add a null in order to allow to use a generic non localized version if no other language is found.
        languages.push(null);
        request.consideredLanguages = new Set(languages);
        request._serializedConsideredLanguagesList = languages.join(':');
    }
    ...
    _saveToCache(key, resolvedRouteDraft){
        ...
        if ( cache !== null ){
            setImmediate(async () => {
                ...
                // TODO: Set "routerID" and "serverID" as tag.
                await cache.set(key, {
                    routeID: routeID,
    ...
    async _resolveByCache(request, key, stats){
        let timeBuffer = process.hrtime(), resolvedRoute = null;
        ...
        if ( cache !== null ){
            ...
            if ( routeData !== null ){
                ...
                if ( typeof routeData !== 'object' || routeData.routeID === '' || typeof routeData.routeID !== 'string' ){
                    ...
                    if ( route === null ){
                        ...
                        const path = routeData.path !== '' && typeof routeData.path === 'string' ? routeData.path : null;
                        const parameters = routeData.parameters !== null && typeof routeData.parameters === 'object' ? routeData.parameters : null;
    ...
    constructor(configuration = null){
        ...
        /**
         * @type {?CORSOptions} [_CORSOptions] An instance of the class "CORSOptions" containing the CORS configuration to use globally.
         *
         * @protected
         */
    ...
    configure(configuration){
        ...
        if ( configuration.hasOwnProperty('CORSOptions') && configuration.CORSOptions instanceof CORSOptions ){
            this._CORSOptions = configuration.CORSOptions;
        }
        return this;
    }
    ...
    async process(request, response){
        ...
        let resolvedRoute = null;
        if ( request.preventRouteProcessing !== true ){
            const stats = RouteProcessor._generateRouteResolutionStatsObject();
            if ( request.isPreflightRequest === true ){
                const originalMethod = request.method;
                request.method = request.preflightRequestMethod;
                resolvedRoute = await this._resolveRoute(request, stats);
            ...
            request.preventRouteExecution = request.isPreflightRequest === true;
            //
            request.CORSOptions = resolvedRoute.getRoute().getCORSOptions();
            if ( request.CORSOptions === null ){
                request.CORSOptions = resolvedRoute.getRouter().getCORSOptions();
                if ( request.CORSOptions === null ) {
                    request.CORSOptions = this._CORSOptions;
                }
            }
        }
        return resolvedRoute;
