...
class AuthorizationProcessor extends Processor {
    ...
    async _processCSRF(request, response, CSRFTokenRequired){
        request.CSRFTokenVerificationStatus = null;
        let CSRFTokenObject = request._CSRFToken;
        if ( CSRFTokenObject === null || typeof CSRFTokenObject !== 'object' ){
            CSRFTokenObject = await this.attachCSRFToken(request, response);
        }
        if ( CSRFTokenObject !== null ){
            ...
            if ( this._CSRFFieldName !== null ){
                ...
                if ( tokenValue !== null ){
                    ...
                    request.CSRFTokenVerificationStatus = this._CSRFTokenStorage.verify(CSRFTokenObject.id, tokenValue);
     ...
     */
    attachCSRFToken(request, response){
        ...
        if ( request.cookies instanceof Map ){
            ...
            if ( CSRFTokenObject === null ){
                CSRFTokenObject = this._CSRFTokenStorage.create(this._CSRFTokenTTL);
                // Save the created token's ID as a cookie on the client side.
                response.setCookie(AuthorizationProcessor.CSRF_ID_COOKIE_NAME, CSRFTokenObject.id, {
                    HTTPOnly: true,
                    secure: request.secure
                });
            }
            request._CSRFToken = CSRFTokenObject;
        }
    ...
    async process(request, response){
        ...
        // Adds helper functions provided by this processor to the given request object.
        HelperRepository.inject(request, 'com.lala.server.processor.AuthorizationProcessor.request', {
            bind: [this],
            context: {
                request: request,
                response: response
