...
class RequestProcessor extends Processor {
    ...
    _addRequestProperties(request){
        ...
        let index = request.completeHost.indexOf(':');
        request.host = index === -1 ? request.completeHost : request.completeHost.substr(0, index);
        request.fullURL = request.completeHost + request.url;
        request.originalURL = request.url;
        request.originalFullURL = request.completeHost + request.originalURL;
        request.preventRouteProcessing = request.preventRouteExecution = false;
        index = request.url.indexOf('?');
        request.path = index > 0 ? request.url.substr(0, index) : request.url;
        request._queryStartIndex = index;
    ...
    _applyURLMapping(request){
        request.mapping = Object.create(null);
    ...
    async process(request, response){
        request.URLLength = Buffer.byteLength(request.url);
