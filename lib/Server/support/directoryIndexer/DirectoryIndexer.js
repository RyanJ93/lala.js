 ...
 * @property {View} view An instance of the class "View" representing the HTML page used to display entries.
...
class DirectoryIndexer extends BaseDirectoryIndexer {
     ...
     *
     * @param value An integer number greater than zero representing the size value in bytes.
     *
     * @returns {string} A string representing the generated representation.
     *
     * @protected
     */
    static _toHumanReadableValue(value){
    ...
    static getDefaultConfiguration(){
        ...
        const view = new View(__dirname + '/../../resources/default_directory_indexer.ejs');
    ...
    async _addFileInfo(list){
        if ( this._showFileInfo === true ){
            ...
            for ( let i = 0 ; i < length ; i++ ){
                ...
                list[i].size = DirectoryIndexer._toHumanReadableValue(info[i].size);
    ...
    async _getFileList(){
        ...
        entries = entries.map((entry) => {
            return {
                filename: entry,
                url: ( this._aliasPath === '/' ? this._route.compile(entry) : ( this._aliasPath + this._route.compile(entry) ) ),
                sortable: {
                    name: entry
                },
                searchable: {
    ...
    /**
     * Generates a clone of the view that has been defined.
     *
     * @returns {View} The cloned view instance.
     *
     * @protected
     */
    _generateViewObject(){
        // Clone the view object that has been defined and set it up according to computed parameters.
        return this._view === null ? null : Object.assign(Object.create(Object.getPrototypeOf(this._view)), this._view);
    }
    ...
    constructor(configuration = null){
        ...
        /**
         * @type {View} _view An instance of the class "View" representing the HTML page used to display entries.
         *
         * @protected
         */
        this._view = new View(__dirname + '/../resources/default_directory_indexer.ejs');
    ...
    configure(configuration){
        ...
        if ( configuration.hasOwnProperty('view') && configuration.view instanceof View ){
            this._view = configuration.view;
        }
     ...
     *
     * @returns {View} An instance of the class view representing the view being used.
     */
    getView(){
        return this._view;
    }
    ...
    async index(path, request, response){
        ...
        const entries = await this._getFileList();
        let ret;
        if ( this._raw === true ){
            ret = entries;
        }else{
            ret = this._generateViewObject();
            if ( ret !== null ){
                // TODO: Remove "_request" and "_constants" once View refactoring has been completed.
                ret.setData({
                    files: entries,
