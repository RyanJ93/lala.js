...
class DirectoryIndexer extends BaseDirectoryIndexer {
     ...
     */
    async _getFileList(){
        const routePath = this._route.getPath();
        // List all the files contained in current directory.
        let entries = await filesystem.promises.readdir(this._path);
        entries = entries.map((entry) => {
            return {
                filename: entry,
                url: ( this._aliasPath === routePath ? this._route.compile(entry) : ( this._aliasPath + this._route.compile(entry) ) ),
                sortable: {
                    name: entry
                },
                searchable: {
    ...
    async index(path, request, response){
        ...
        if ( this._raw === true ){
            ...
            // Generate a clone of the view defined and set up parameters.
            ret = this._view.clone().setParams({
                files: files,
