...
class HTTPSerializer extends StreamSerializer {
    ...
    async _processObject(obj){
        ...
        if ( obj instanceof Response ){
            ...
            const data = output = await obj.apply(this._context.getRequest(), response);
            ...
            this._contentType = typeof contentType === 'string' && contentType !== '' ? contentType : 'application/octet-stream';
        }else if ( obj instanceof ParametrizedView ){
            // Render the view and returns the compiled HTML content.
            output = await obj.renderAsStream(null, this._context);
