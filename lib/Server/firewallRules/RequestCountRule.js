...
/**
 * @typedef requestCounters An object containing all the properties related to the global and request's counters.
 *
...
/**
 * @typedef URLLimit An object representing limit settings for an URL.
 *
...
class RequestCountRule extends FirewallRule {
    ...
    async _getParameters(request){
        ...
        if ( cacheHandler !== null ){
            ...
            const namespace = cacheHandler.getNamespace();
            cacheHandler.setNamespace(this._getNamespaceString());
            ...
            parameters = await cacheHandler.getMulti(keys, {
                silent: true
            });
            ...
            // TODO: Remove this block once task number #LALA-12 has been completed.
            cacheHandler.setNamespace(namespace);
        }
        return parameters;
    }
    ...
    async _filterIncomingConnection(request, response){
        ...
        if ( cacheHandler !== null ){
            ...
            let expire = NaN, lrd = NaN, limit = null, count = NaN, retryAfter = null, url = null;
            ...
            if ( !isNaN(expire) && expire > lrd ){
                ...
                setImmediate(async () => {
                    await this.resetClientCounter(identifier, url, true);
                });
            }else{
                ...
                setImmediate(async () => {
                    await this.incrementClientCounter(identifier, url, 1, true);
                });
            }
        }
        return true;
    ...
    async incrementClientCounter(identifier, url = null, amount = 1, logDate = true){
        ...
        const cacheHandler = this._getCacheHandlerObject();
        if ( cacheHandler === null ){
            throw new RuntimeException('No cache handler defined.', 3);
        }
        let processes = [];
    ...
    async resetClientCounter(identifier, url = null, initialize = false){
        ...
        const cacheHandler = this._getCacheHandlerObject();
        if ( cacheHandler === null ){
            throw new RuntimeException('No cache handler defined.', 2);
        }
        // Generate the base key according to URL defined, if no URL is given, generate the global key for this client.
        const key = url !== null && url !== '' && typeof url === 'string' ? ( identifier + ':' + url ) : ( identifier + ':*' );
        const entries = {};
