...
/**
 * A firewall rule that allows to limit traffic from defined IP addresses.
 */
class IPFilterRule extends FirewallRule {
    /**
    ...
    async _filterIncomingConnection(request, response){
        ...
        const wildcards = IPFilterRule._getIPWildcards(ip);
        let allowed = true;
