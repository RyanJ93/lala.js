...
class RoutedServer extends Server {
    ...
    async _resolveRequestRoute(request, response){
        ...
        await this._runFirewallCheckpoint('request.routeResolution', request, response);
        const processor = this._routeProcessorFactory.craft();
        // Find out what route matches the request path.
        request.resolvedRoute = await processor.process(request, response);
    ...
    async _runMiddlewares(request, response){
        let result = true;
        if ( request.hasOwnProperty('route') && request.route instanceof BaseRoute ){
        ...
        }else if ( request.hasOwnProperty('resolvedRoute') && request.resolvedRoute instanceof ResolvedRoute ){
    ...
    async _processRoute(request, response){
        // Checking if the request can continue according to firewall's rules.
        await this._runFirewallCheckpoint('request.routeProcess', request, response);
        //
        ...
        response.rawOutput = route === null ? null : await route.trigger(request, response).execute(request, response);
