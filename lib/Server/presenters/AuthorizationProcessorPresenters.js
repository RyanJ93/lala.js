...
function CSRFToken(parameters){
    ...
    if ( parameters._request !== null && typeof parameters._request === 'object' ){
        // Get the CSRF token that has been generated for current client or generate a new one on-the-fly.
        const CSRFTokenObject = parameters._request.getCSRFToken();
        if ( CSRFTokenObject !== null ){
            CSRFToken = CSRFTokenObject.token;
        }
    }
    return CSRFToken;
}
