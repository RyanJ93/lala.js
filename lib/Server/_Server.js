...
const Request = require('./processors/RequestProcessor');
...
class _Server extends EventEmitter{
    ...
    useRandomPort(options){
        let port = _Server.getRandomPort(options);
        if ( port === null ){
            //TODO: Add exception.
        }
        this.setPort(port);
    ...
    _createHTTPServer(tls){
        ...
        this.server = ( tls === true ? https : http ).createServer(options, async (request, handler) => {
            ...
            Request.prepareResponse(handler);
            ...
            await _Server.defaultHandler(request, handler, {
    ...
    /**
     * Returns the _Server object, basically the original object created from the "http", the "https" or the "websocket" module.
     *
