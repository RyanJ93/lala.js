...
const { generateToken } = require('../Helpers/helpers/BuiltInHelpers');
const {
...
class DigestHTTPAuthentication extends HTTPAuthenticator {
     ...
     * 
     * @param {Object.<string, string>} components An object containing as key the property name and as value the corresponding value as a string.
     *
     ...
     *
     * @returns {?object} An object having as key the component name and as value its value, if header content is not valid os not eligible will be returned null instead.
     *
     * @protected
     */
    static _parseHeader(authorization){
        ...
        let components = {};
        for ( let i = 0 ; i < length ; i++ ){
            // Locate key/value separator in current component.
            const index = blocks[i].indexOf('=');
            if ( index > 0 ){
                // Extract component key and value.
                const key = blocks[i].substr(0, index);
                const value = blocks[i].substr(index + 1);
                if ( key !== '' && value !== '' ){
                    // Validate current component in order to check if it is a valid key/value pair separated by the "=" sign.
                    components[key] = value.charAt(0) === '"' ? value.substr(1, value.length - 2) : value;
                }
            }
        }
    ...
    _buildAuthDigest(context, components, credentials, request){
        ...
        if ( context.algorithm === 'MD5-sess' ){
            cocktail = components.username + ':' + this.getRealm() + ':' + credentials.getPassword();
    ...
    constructor(){
        ...
        // Disable custom comparison function.
        delete this.setPasswordCompareFunction;
        delete this.getPasswordCompareFunction;
    }
     ...
     *
     * @returns {Promise<?AuthenticationResult>} An object containing both the authenticated user and its session.
     *
     ...
     * @throws {UserNotFoundHTTPException} If no user matching the username specified has been found.
     * @throws {TokenExpiredHTTPException} If client token has expired or removed.
    ...
    async authenticateRequest(request, response){
        ...
        if ( !request.hasOwnProperty('authMethod') || request.authMethod !== 'digest' ){
        ...
        if ( typeof session === 'undefined' || session.expire < ( new Date() ) || session.attemptsLeft <= 0 ){
            ...
            throw new TokenExpiredHTTPException('The given nonce token has expired.', 4);
        }
        ...
        const responseBuffer = Buffer.from(components.response, 'hex');
        // Compare hashes in secure way.
        if ( !crypto.timingSafeEqual(digest, responseBuffer) ){
            // Inform the client that a new authentication session must be initialized.
            await this.requestAuthentication(request, response);
        ...
        const user = userObject === null ? new AuthenticationResult(identifier) : new AuthenticationResult(userObject.identifier, {
        ...
        session.setUser(user.getUserData()).setPending(false).setContext(null).register();
    ...
    async requestAuthentication(request, response){
        ...
        if ( session.getContext() === null ){
            // As the session found has no context attached, generate a new session and restart authentication process.
            session = await this._generateSession();
            stale = true;
        }
        const context = session.getContext();
        ...
        if ( stale ){
            // Inform the client that a new session has been generated as authentication tokens stored should be refreshed.
            header += ', stale="TRUE"';
        }
