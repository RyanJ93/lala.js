...
class BasicHTTPAuthenticator extends HTTPAuthenticator {
     ...
     *
     * @returns {Promise<?{username: string, userData: *}>} An object representing the user found or null if given password is not correct.
     *
    ...
    async validate(identifier, password){
        ...
        if ( crypto.timingSafeEqual(passwordDigest, validPasswordDigest) ){
            ...
            user = userObject === null ? {
                ...
                identifier: userObject.identifier,
     ...
     *
     * @returns {Promise<{username: string, userData: *}>} An object containing user information such as its username and the custom user properties defined, namely "userData".
     *
