...
/**
 * Implements validation rule that ensures a field is a valid array containing unique entries.
 */
class DistinctValidationRule extends ValidationRule {
    /**
    ...
    validate(value, validator, params){
        return Array.isArray(value) && value.length === Array.from(new Set(value)).length;
    }
     ...
     */
    getMessage() {
        return 'Field {fieldName} must be a valid array containing unique entries.';
    }
}
 ...
 * @default
 */
Object.defineProperty(DistinctValidationRule, 'RULE_NAME', {
    value: 'distinct',
    writable: false
});

module.exports = DistinctValidationRule;
