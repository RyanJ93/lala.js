...
class TypeProvider extends Provider {
    ...
    static async setup(){
        ...
        TypeRepository.register('tuple', (value, subType) => {
            ...
            const values = value.split(','), length = types.length, mapping = {}, properties = {};
        ...
        TypeRepository.register('enum', (value, subType) => {
            const allowedValues = subType === null ? [] : subType.split(',');
            return allowedValues.indexOf(value) >= 0 ? value : null;
        });
        TypeRepository.register('JSON', (value) => {
            try{
