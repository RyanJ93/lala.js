    ...
    static async handle(request, handler, options){
        ...
        for ( let type in resources ){
            ...
            for ( let key in resources[type] ){
                if ( resources[type].hasOwnProperty(key) && url.indexOf(key) === 0 ){
        ...
        if ( route === null ){
            ...
            for ( let type in routes ){
                ...
                for ( let key in routes[type] ){
                    ...
                    if ( routes[type][key].method === method && routes[type][key].route === url ){
                    ...
                    if ( routes[type][key].method === '*' && routes[type][key].route === url ){
                        route = routes[type][key];
                    }
                }
                if ( found ){
        ...
        if ( resource !== null ){
            ...
            let path = url.substr(resource.length);
