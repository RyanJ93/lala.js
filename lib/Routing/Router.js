...
class Router{
    ...
    static async _runMiddlewares(route, request, handler, options){
        ...
        let next = async (_request = request, _handler = handler) => {
            ...
            while ( identifier === null && typeof identifier !== 'undefined' ){
                ...
                identifier = index === 0 && first === null ? identifiers[scope][0] : identifiers[scope][index + 1];
                first = null;
                ...
                identifier = null;
            }console.log(identifier, elements[scope], index);
        ...
        for ( let i = 0 ; i < elements.length ; i++ ){
            ...
            // Pick the first one but keep going on in order to fetch all the middlewares' keys.
            if ( first === null && identifiers[i].length > 0 && elements[i].hasOwnProperty(identifiers[i][0]) ){
                first = elements[i][identifiers[i][0]];
                scope = i;
            }
        }
    ...
    static async _runParamMiddlewares(route, request, handler, options){
        ...
        for ( let i = 0 ; i < elements.length ; i++ ){
            ...
            // Pick the first one but keep going on in order to fetch all the middlewares' keys.
            if ( first === null && identifiers[i].length > 0 && elements[i].handlers.hasOwnProperty(identifiers[i][0]) ){
                first = elements[i].handlers[identifiers[i][0]].handler;
                scope = i;
            }
        }
        // If no middleware is going to be executed the request can be processed.
    ...
    static async _processRoute(route, request, handler, options){
        ...
        if ( route.location !== '' && typeof route.location === 'string' ){
            ...
            let path = url.substr(route.location.length);
