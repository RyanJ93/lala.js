 ...
 * @property {boolean} [allowConditionalRequests=true] If set to "true" client provided version information will be checked in order to prevent sending a resource already owned by the client.
 */
...
class ResourceRoute extends BaseRoute {
     ...
     *
     * @param {ResourceRouteOptions} options An object containing the configuration options the given route will be configured with.
     * @param {ResourceRoute} instance A route instance.
     *
     * @protected
     */
    static _configureInstance(options, instance){
        super._configureInstance(options, instance);
        instance.setDirectoryListing(options.hasOwnProperty('directoryListing') && options.directoryListing === true);
        if ( options.hasOwnProperty('directoryIndexerFactory') && options.directoryIndexerFactory instanceof BaseDirectoryIndexerFactory ){
            instance.setDirectoryIndexerFactory(options.directoryIndexerFactory);
        }
        instance.setServeHiddenFiles(options.hasOwnProperty('serveHiddenFiles') && options.serveHiddenFiles === true);
        const allowConditionalRequests = !options.hasOwnProperty('allowConditionalRequests') || options.allowConditionalRequests !== false;
        instance.setAllowConditionalRequests(allowConditionalRequests);
    }
    ...
    static craft(path, location, options = null){
        ...
        if ( options !== null ){
            ResourceRoute._configureInstance(options, route);
        }
        return route;
    }
    ...
    async execute(request, response){
        ...
        if ( stat.isDirectory() ){
            ...
            responseObj.setAllowConditionalRequests(this._allowConditionalRequests);
        }
        return responseObj;
    }
}
