 ...
 * @property {?boolean} [withCSRF=false] If set to "true" a CSRF token will be generated once this route gets triggered by a client request.
 * @property {?boolean} [requireCSRF=false] If set to "true" and no valid CSRF token is provided by the client side the connection will be dropped sending back the error code 403 (requires the withCSRF option to be set to "true").
 ...
 * @property {?CORSOptions} [CORSOptions] An instance of the class "CORSOptions" containing the CORS configuration for this route.
 */
 ...
 *
 * @mixes Middlewares, Permissions, Policies, HeaderManagement, CORSManagement
 *
 ...
 * @abstract
 */
class BaseRoute extends mixin(Middlewares, Permissions, Policies, HeaderManagement, CORSManagement, EventEmitter) {
    /**
     ...
     *
     * @param {BaseRouteOptions} options An object containing the configuration options the given route will be configured with.
     * @param {BaseRoute} instance A route instance.
     *
     * @protected
     */
    static _configureInstance(options, instance){
        if ( options.hasOwnProperty('middlewares') && options.middlewares !== null && typeof options.middlewares === 'object' ){
            instance.setMiddlewares(options.middlewares);
        }
        if ( options.hasOwnProperty('authenticator') && options.authenticator instanceof Authenticator ){
            instance.setAuthenticator(options.authenticator);
        }
        ...
        if ( options.hasOwnProperty('permissions') ){
            if ( options.permissions instanceof Set ){
                instance.setPermissions(options.permissions);
            }else if ( Array.isArray(options.permissions) ){
                instance.setPermissionsAsArray(options.permissions);
            }
        }
        if ( options.hasOwnProperty('policies') ){
            if ( options.policies instanceof Map ){
                instance.setPolicies(options.policies);
            }else if ( options.policies !== null && typeof options.policies === 'object' ){
                instance.setPoliciesAsObject(options.policies);
            }
        }
        if ( options.hasOwnProperty('headerManagers') && Array.isArray(options.headerManagers) ){
            instance.setHeaderManagers(options.headerManagers);
        }
        if ( options.hasOwnProperty('CORSOptions') && options.CORSOptions instanceof CORSOptions ){
            instance.setCORSOptions(options.headerManagers);
        }
        const auth = options.hasOwnProperty('auth') ? ( options.auth === true ? true : ( options.auth === false ? false : null ) ) : null;
        instance.setAuth(auth);
        instance.setWithCSRF(options.hasOwnProperty('withCSRF') && options.withCSRF === true);
        instance.setRequireCSRF(options.hasOwnProperty('requireCSRF') && options.requireCSRF === true);
    }
     ...
     *
     * @param {?string} [name] A string containing the name that was previously assigned to this route used in order to cleanup old data before indexing this route again.
     *
    ...
    constructor(){
        ...
        /**
         * @type {boolean} [_withCSRF=false] If set to "true" a CSRF token will be generated once this route gets triggered by a client request.
         *
         * @protected
         */
        this._withCSRF = false;

        /**
         * @type {boolean} [_requireCSRF=false] If set to "true" and no valid CSRF token is provided by the client side the connection will be dropped sending back the error code 403.
         *
         * @private
         */
        this._requireCSRF = false;
    }
    ...
    /**
     * Sets if a CSRF token should be generated once this route gets triggered by a client request, this method is chainable.
     *
     * @param {boolean} withCSRF If set to "true" CSRF token will be prepared once this route gets triggered.
     *
     * @returns {BaseRoute}
     */
    setWithCSRF(withCSRF){
        this._emitUpdateEvent('withCSRF', withCSRF);
        this._withCSRF = withCSRF === true;
        this.emit('updated', 'withCSRF');
        return this;
    }

    /**
     * Returns if a CSRF token should be generated once this route gets triggered by a client request.
     *
     * @returns {boolean} If CSRF token support has been turned on will be returned "true".
     */
    getWithCSRF(){
        return this._withCSRF === true;
    }

    /**
     * Sets if a CSRF token is expected to be sent by the client side, this method is chainable.
     *
     * @param {boolean} requireCSRF If set to "true" a valid CSRF token must be sent by the client and it will be validated.
     *
     * @returns {BaseRoute}
     */
    setRequireCSRF(requireCSRF){
        this._emitUpdateEvent('requireCSRF', requireCSRF);
        this._requireCSRF = requireCSRF === true;
        this.emit('updated', 'requireCSRF');
        return this;
    }

    /**
     * Returns if a CSRF token is expected to be sent by the client side.
     *
     * @returns {boolean} If CSRF token is required will be returned "true".
     */
    getRequireCSRF(){
        return this._requireCSRF === true;
    }
     ...
     * @param {module:http.ServerResponse} response An instance of the built-in class "ServerResponse" representing the response that will be sent back to the client.
     *
     * @returns {Promise<*>} Some data returned by the route's controller function that will be returned to the client as a request response.
     *
     * @throws {NotCallableException} If this method is called without been overridden and implemented.
     *
     * @abstract
 ...
 * @readonly
 */
const SUPPORTED_METHODS = Object.freeze(['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'SEARCH', 'OPTIONS']);
