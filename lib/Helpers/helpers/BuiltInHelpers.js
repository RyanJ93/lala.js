...
function generateToken(length){
    ...
    return new Promise((resolve, reject) => {
        ...
        crypto.randomBytes(length / 2, (error, bytes) => {
            ...
            // Create a hexadecimal string representation.
...
/**
 * Generates a cryptographically secure token in synchronous way.
 *
 * @param {number} length An integer number greater than zero representing the token length.
 *
...
function generateTokenSync(length){
    ...
    try{
        ...
        // Create and return a hexadecimal string representation.
        return bytes.toString('hex');
    }catch(ex){
        throw new RuntimeException('Unable to generate the token.', 1, ex);
    }
}

/**
 * Generates a NON cryptographically secure token.
 *
 * @param {number} length An integer number greater than zero representing the token length.
 *
 * @returns {string} A string containing the generated token.
 ...
 * @throws {InvalidArgumentException} If an invalid length is given.
 */
function generatePlainToken(length){
    if ( length === null || isNaN(length) || length <= 0 ){
        throw new InvalidArgumentException('Invalid token length.', 1);
    }
    let token = '';
...
module.exports.registerHelpers = () => {
    ...
    HelperRepository.register('generateTokenSync', generateTokenSync);
    HelperRepository.register('generatePlainToken', generatePlainToken);
...
module.exports.generateTokenSync = generateTokenSync;
module.exports.generatePlainToken = generatePlainToken;
